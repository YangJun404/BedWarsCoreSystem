...
# 创造微观模型
execute as @e[type=marker,tag=map_center] at @s positioned ~ 45 ~24 run function debug:map_import/entity_event/init_command_area
...
tellraw @s [{"text": "[⚠] ","color": "yellow","bold": true},{"text": "请","bold": false},{"text": "回地图本体","bold": false,"underlined": true,"clickEvent": {"action": "run_command","value": "/tp @s @e[type=marker,tag=map_importer,tag=map_center,limit=1]"}},{"text": "，在y=44处，将","bold": false},{"text": "command_area:","bold": false,"underlined": true},{"score":{"name": "$map_id","objective": "temp"},"bold": false,"underlined": true},{"text": "填入结构方块并保存","bold": false}]
tellraw @s {"text": "---------------------","color": "yellow"}

scoreboard players reset $map_id temp

# 发送下一步提示
scoreboard players set @s mi_mem 520
tellraw @s ""
